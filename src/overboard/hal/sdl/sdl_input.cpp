...
void SDL_Input::pump() {
    ...
    while (SDL_PollEvent(&ev)) {
        ...
        // Handle mouse events on the keyboard window
        ...
        } else if (ev.type == SDL_MOUSEBUTTONUP && ev.button.windowID == m_kbd_window_id) {
            int idx = hit_test(ev.button.x, ev.button.y);
            if (idx >= 0) {
        ...
        // Handle keyboard events (global - not tied to a specific window)
            ...
            // Non-keyboard event (mouse on other windows, window, etc.) - push back for LVGL
            SDL_PushEvent(&ev);
        }
    }
}
